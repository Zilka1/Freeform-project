...
class MainServer:
    ...
    def create_new_db(self, client, password):
        ...
        c.execute(
            'CREATE TABLE drawings (id INTERGER PRIMARY KEY, color TEXT, width INTEGER, pt_list TEXT)')
    ...
    def get_projects_names(self, client):
        ...
        # Filter the db files
        db_files = [file.stem for file in files if file.suffix == ".db"]
        print(db_files)
    ...
    def new_rect_oval(self, client, obj: Rect | Oval, type_):
        ...
        c.execute(f'INSERT INTO {type_}s VALUES (?,?,?,?,?,?,?)',
                  (obj.id, obj.color, obj.width, obj.x1, obj.y1, obj.x2, obj.y2))
